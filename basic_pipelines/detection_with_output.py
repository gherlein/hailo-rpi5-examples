...
# -----------------------------------------------------------------------------------------------
# Detection App with Frame File Output (REAL-TIME with Async Writing)
# -----------------------------------------------------------------------------------------------
# This app extends the standard detection pipeline to write annotated frames to files.
#
# Key Features:
# - Bounding boxes: All detections are drawn with labels and confidence scores
...
class AsyncFrameWriter:
    ...
    def _writer_thread(self):
        ...
        while self.running:
            try:
                ...
                cv2.putText(frame_copy, f"Detections: {detection_count}",
    ...
    def write_async(self, output_path, frame_rgb, frame_count, detection_count, detections):
        """Queue a frame for asynchronous processing and writing."""
        try:
            # Non-blocking put - just queue the raw frame data
            # NO copying or processing here - all happens in background thread
            self.write_queue.put_nowait((output_path, frame_rgb, frame_count, detection_count, detections))
            return True
        except queue.Full:
...
class DetectionWithFileOutput(GStreamerDetectionApp):
    def __init__(self, app_callback, user_data, parser=None):
        """
        Initialize the detection app with frame file output.
        ...
        parser.add_argument(
            "--output", "-o",
            type=str,
            default="/tmp/latest_frame.jpg",
            help="Output file path (default: /tmp/latest_frame.jpg)"
        )
        parser.add_argument(
            "--no-display",
            action="store_true",
        ...
        # Call parent init which will parse args and call create_pipeline()
        super().__init__(app_callback, user_data, parser)

    def get_pipeline_string(self):
        """
        Override pipeline to write frames via callback.
        Uses standard display pipeline (or fakesink if no-display).
        """
...
# -----------------------------------------------------------------------------------------------
def frame_writer_callback(pad, info, user_data):
    """
    ...
    - Extract detection metadata (labels, bboxes, confidence)
    ...
    # Extract detection data for background thread processing
    ...
    if user_data.write_frame(frame_copy, user_data.frame_count, detection_count, detection_list, capture_timestamp):
        if user_data.write_count % 30 == 0:  # Print every 30 frames
            if user_data.write_all:
                ...
                print(f"Frame {user_data.frame_count}: Queued {current_file} ({detection_count} detections)")
            elif user_data.copy_all:
                print(f"Frame {user_data.frame_count}: Queued to {user_data.output_path} (copy-all mode, {detection_count} detections)")
            else:
                print(f"Frame {user_data.frame_count}: Queued to {user_data.output_path} ({detection_count} detections)")
...
if __name__ == "__main__":
    ...
    temp_user_data = FrameWriterCallback("/tmp/latest_frame.jpg")

    # Create app with file output (arguments will be parsed inside)
    app = DetectionWithFileOutput(frame_writer_callback, temp_user_data)
    ...
    print("\n" + "="*70)
    print("Detection with Frame File Output")
    print("="*70)
